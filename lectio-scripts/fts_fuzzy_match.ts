 ...
 * @param {*} str string
 */
export function fuzzyMatchSimple(pattern: string, str: string): [boolean, number] {
	let patternIdx = 0;
	let strIdx = 0;
 ...
 */
export function ftsFuzzyMatchAsync(
	this: ftzz,
	matchFn: (pattern: string, str: string) => [boolean, number],
	pattern: string, dataSet: string,
	onComplete: (results: [boolean, number][]) => void
) {
	// performance.now can be very slow depending on platform
	...
	function step() {
		...
		for (; dataIndex < dataSet.length; ++dataIndex) {
			...
			const str = dataSet[dataIndex];
			const result = matchFn(pattern, str);

			if (result[0] == true) results.push(result);
		}
	...
	// Must be called to start matching. I tried to make asyncMatcher auto-start
	// via "var resumeTimeout = step();" However setTimout behaving in an
	// unexpected fashion as onComplete insisted on triggering twice.
	this.start = function () {
		step();
	};
